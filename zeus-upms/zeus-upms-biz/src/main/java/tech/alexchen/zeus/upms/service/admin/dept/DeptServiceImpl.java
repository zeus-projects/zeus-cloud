package tech.alexchen.zeus.upms.service.admin.dept;
...
import tech.alexchen.zeus.upms.controller.admin.dept.vo.DeptRequestVO;
...
import tech.alexchen.zeus.upms.dal.mapper.dept.DeptMapper;
...
public class DeptServiceImpl extends ServiceImpl<DeptMapper, DeptDO> implements DeptService {
    ...
    public List<Tree<Long>> getDeptTreeByParentId(Long parentId) {
        ...
        if (DeptIdEnum.ROOT_DEPT_ID.getId().equals(parentId)) {
            ...
            if (parentDept == null) {
                throw ExceptionUtil.newException(ErrorCodeConstants.DEPT_PARENT_NOT_EXIST);
            }
    ...
    private void checkDeptExists(Long id) {
        ...
        if (dept == null) {
            throw ExceptionUtil.newException(ErrorCodeConstants.DEPT_NOT_FOUND);
        }
    }
    ...
    private void checkParentDeptEnable(Long id, Long parentId, Integer level) {
        ...
        if (!CommonStatusEnum.ENABLE.getStatus().equals(parentDept.getStatus())) {
            throw ExceptionUtil.newException(ErrorCodeConstants.DEPT_DISABLE);
        }
        // 不能设置自己为父部门
        if (id != null && parentDept.getId().equals(id)) {
            throw ExceptionUtil.newException(ErrorCodeConstants.DEPT_PARENT_ERROR);
        }
        // 父部门不能是原来的子部门
        if (level != null && parentDept.getLevel() < level) {
            throw ExceptionUtil.newException(ErrorCodeConstants.DEPT_PARENT_IS_CHILD);
        }
    }
    ...
    private void checkDeptNameUnique(Long id, String deptName, Long parentId) {
        ...
        if (id == null) {
            throw ExceptionUtil.newException(ErrorCodeConstants.DEPT_NAME_DUPLICATE);
        }
