...
import tech.alexchen.zeus.system.convert.SysDeptConverter;
import tech.alexchen.zeus.system.entity.SysDept;
import tech.alexchen.zeus.system.mapper.SysDeptMapper;
import tech.alexchen.zeus.system.service.SysDeptService;
...
public class SysDeptServiceImpl implements SysDeptService {
    ...
    public Long save(SysDeptSaveDTO dto) {
        if (!SysConstant.ROOT_DEPT_ID.equals(dto.getParentId()) && sysDeptMapper.isParentDeptNotExists(dto.getParentId())) {
            throw new ResponsiveRuntimeException(SystemResponseCode.SYS_DEPT_PARENT_NOT_EXISTS);
        }
        if (sysDeptMapper.isDeptNameExists(dto.getName(), null)) {
            throw new ResponsiveRuntimeException(SystemResponseCode.SYS_DEPT_NAME_DUPLICATE);
        }
