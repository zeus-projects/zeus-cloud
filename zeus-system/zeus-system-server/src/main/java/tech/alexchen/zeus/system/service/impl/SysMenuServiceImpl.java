...
import tech.alexchen.zeus.system.api.constant.SysConstant;
...
import tech.alexchen.zeus.system.convert.SysMenuConverter;
import tech.alexchen.zeus.system.entity.SysMenu;
import tech.alexchen.zeus.system.mapper.SysMenuMapper;
import tech.alexchen.zeus.system.service.SysMenuService;
...
public class SysMenuServiceImpl implements SysMenuService {
    ...
    public Long saveMenu(SysMenuSaveDTO dto) {
        if (!SysConstant.ROOT_MENU_ID.equals(dto.getParentId()) && sysMenuMapper.isParentMenuNotExists(dto.getParentId())) {
            throw new ResponsiveRuntimeException(SystemResponseCode.SYS_MENU_PARENT_NOT_EXISTS);
        }
        // 检查同级别下是否存在相同名称的菜单
        if (sysMenuMapper.isDeptNameExists(dto.getName(), dto.getParentId(), null)) {
            throw new ResponsiveRuntimeException(SystemResponseCode.SYS_MENU_NAME_DUPLICATE);
        }
    ...
    public void removeMenuById(Long id) {
        if (!sysMenuMapper.canMenuDeleted(id)) {
            throw new ResponsiveRuntimeException(SystemResponseCode.SYS_MENU_HAS_SUBMENU);
        }
